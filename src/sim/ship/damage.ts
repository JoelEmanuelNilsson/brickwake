...
export const buildDamageGraph = (spec: ShipSpec, ship: GeneratedShip): DamageGraph => {
  ...
  ship.parts.forEach((p, i) => {
    ...
    // The rig counts as upper works all the way down: a mast step on the waist deck sits below the line.
    upperWorks[i] = i >= ship.rigFrom || p.y >= spec.upperWorks + Math.round(sampleCurve(spec.sheer, p.x + fx / 2)) ? 1 : 0
  })
