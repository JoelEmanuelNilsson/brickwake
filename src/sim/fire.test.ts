...
test("fire that burns a ship to 0 HP sinks it, credited to the ship that set it; it goes out once the ship is under", () => {
  ...
  const under = run(state, tuning.sinking.seconds).state
  expect(ship(under, b).life._tag).toBe("sunk")
  expect(ship(under, b).fires).toHaveLength(0)
})
