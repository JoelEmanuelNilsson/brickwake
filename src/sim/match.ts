 ...
 * due in the ripple fire (with recoil), ships move and push apart, balls fly and hit, splash or set fires, fires burn, sinks score, sinking
...
export const stepMatch = (
  ...
  const founder = (index: number, localPoint: Vec3, time: number, by: ShipId): MatchEvent => {
    ...
    const credited = moved.some((ship) => ship.id === by) ? by : undefined
    if (state.phase._tag === "playing") ({ ships: moved, teamSinks } = scoreSink(state.rules, { ships: moved, teamSinks }, target.id, credited))
    return { _tag: "shipSunk", tick: state.tick, time, shipId: target.id, by: credited }
  }
  ...
  moved = moved.map((ship) =>
    ship.life._tag === "sinking" && end >= ship.life.since + sinkSeconds
      ? { ...ship, life: { _tag: "sunk", respawnAt: ship.life.since + sinkSeconds + respawnSeconds }, fires: [] }
      : ship,
  )
  for (let index = 0; index < moved.length; index++) {
    const ship = moved[index]!
