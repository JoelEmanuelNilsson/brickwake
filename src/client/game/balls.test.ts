...
test("a ripple broadside fires gun by gun and each ball ends on its own", () => {
  ...
  balls.update(1.07, eye)
  expect(log).toEqual(["fired 10", "fired 11"])
  balls.update(1.7, eye)
...
test("a far ball is drawn larger than a near one, and every drawn ball trails a streak", () => {
  const { balls } = record()
  balls.onEvent(fired(1, 1))
  const drawnScale = (from: Vector3) => {
    balls.update(1.2, from)
    const m = new Matrix4()
    balls.mesh.getMatrixAt(0, m)
    return new Vector3().setFromMatrixScale(m).x
  }
  expect(drawnScale(new Vector3(0, 3, 20))).toBe(1)
  expect(drawnScale(new Vector3(0, 3, 400))).toBeGreaterThan(2)
  expect(balls.trails.count).toBe(balls.mesh.count)
})
