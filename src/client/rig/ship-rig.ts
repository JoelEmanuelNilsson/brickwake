...
export const buildRigGeometry = (layout: RigLayout): RigGeometry => {
  ...
  const flag = { position: [] as Array<number>, pole: [] as Array<number>, flag: [] as Array<number>, uv: [] as Array<number>, index: [] as Array<number> }
  const addFlag = (pole: V3, length: number, height: number, taper: number, region: AtlasRegion, amplitude: number) => {
  ...
  for (const m of masts) {
    if (m === tallest) addFlag(poleTop(m), 2.2, 1.2, 0, "flag", 0.22)
    else addFlag(poleTop(m), 4.2, 0.5, 0.85, "pennant", 0.35)
  }
  const [ensignX, ensignY] = layout.ensign
  const ensignTop: V3 = [ensignX - 0.9, ensignY + 2.8, 0]
  addFlag(ensignTop, 1.9, 1.05, 0, "ensign", 0.2)
  ...
  const line = { position: [] as Array<number>, normal: [] as Array<number>, color: [] as Array<number>, index: [] as Array<number> }
  ...
  const shroudSets: Array<{ readonly lows: ReadonlyArray<(s: number) => V3>; readonly highs: ReadonlyArray<(s: number) => V3> }> = []
  masts.forEach((m, k) => {
    const ring = ringOf(m)
    ...
    if (upper !== undefined) {
      ...
      shroudSets.push({ lows: topLows, highs: topHighs })
    }
  ...
  for (const set of shroudSets) mirrored((s) => set.lows.forEach((low, i) => ropeLine(low(s), set.highs[i]?.(s) ?? low(s), 0.05)))
  tube(heel, tip, 0.17, 0.09, 8, wood)
  for (const m of masts) tube([m.x, m.top - 0.2, 0], poleTop(m), 0.07, 0.045, 6, wood)
...
void flagShape(out vec3 p, out vec3 n) {
  ...
  p = pole + along * s + vec3(0.0, flag.y, 0.0) + side * w;
